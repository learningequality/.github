...
Before/after holidays, toggle `enabled:` for `holiday-message` in [`automation-registry.yml`](../automation-registry.yml) and regenerate (see [automation](./automation.md)) - no per-repo changes needed.
